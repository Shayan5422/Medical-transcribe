...
selectUpload(upload_id: number): void {
  ...
  this.http.get<any>(`https://51.15.224.218:8000/get-transcription/${upload_id}`, { headers }).subscribe(
    response => {
...
uploadAudio(file: File): void {
  ...
  this.http.post<any>('https://51.15.224.218:8000/upload-audio/', formData, { headers }).subscribe(
    response => {
  ...
  downloadTranscriptionFile(upload_id: number): void {
    ...
    const url = `https://51.15.224.218:8000/download-transcription/${upload_id}`;
  ...
  downloadAudioFile(upload_id: number): void {
    ...
    const url = `https://51.15.224.218:8000/download-audio/${upload_id}`;
    this.http.get(url, { headers, responseType: 'blob' }).subscribe(blob => {
      const a = document.createElement('a');
      const objectUrl = URL.createObjectURL(blob);
      a.href = objectUrl;
  ...
  fetchHistory(): void {
    ...
    this.http.get<any>('https://51.15.224.218:8000/history/', { headers }).subscribe(
      response => {
